...
| **Message box: user-relevant only (AUDIT)** | **Now (non-logic)** | Confirm the in-mission box shows only messages relevant to THIS player, not every player's items. Showing everything should not be the default; if we want it, it becomes a setting on the full text client (below). |
| **Full in-game text client (replace current display)** | **Investigate effort** | Scope how much work it is to build a real in-game text client (send hints/commands from in-game) in place of the current read-only message box. Not committed; the player can use an external text client meanwhile. |
| **Traps** | **Test now (feasibility)** | Prototype whether we can spawn: a random spore, random creep, an energy drain, and a short "enemies emit more" burst (~one wave). If feasible, they become optional trap items later. |
...
| **Goal variants (yaml Choice)** | **Consider (with options)** | Low-effort once an options dataclass exists (e.g. beat M20 vs all-20). |
| **Tiered per-mission objective checks** | **Consider (with survey)** | More locations per mission; needs the per-mission objective survey. |
