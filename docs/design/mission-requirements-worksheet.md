...
## Instance identity (2026-09-08)

Counted checks moved from "the Nth one you completed" to a SPECIFIC structure,
identified by its map cell and numbered by sorting `(cellY, cellX)` ascending -
the same order `InstanceIndex.Assign` uses in the mod. That invalidated every
table keyed to instance number, so the cells were dumped
(`tools/instance-dump.sh`, raw output in `.aptest/instance-dump.txt`) and the
designer's map reading mapped onto them. The mapping is recorded per mission
below so nobody has to re-derive it.

Orientation, derived rather than assumed: **left is low X, top is high Y.** It
falls out of Sequence, where the four described groups match the dumped cells
exactly by unit type as well as by count - "the 4 enemies in the right bottom
(skimmer, 2 emitters, 1 blob)" are the four at Y 25-55, and "the top right 3
(blob, 2 emitters)" are the three at Y 132-144. Shattered agrees independently:
the totem called "top left" is the low-X high-Y one, and it is also the only one
near the nullify cluster.

What the cell is FOR: naming, not detection. Whether a structure is done is read
off the structure itself - `IsSuppressed()` for an enemy, `totemComplete` for a
totem - and the cell only decides whether that structure is "Totem 1" or
"Totem 3". Caches are the sole exception, because collecting one DESTROYS it:
Farsite went from mustCollect 2 to 1 and from 2 InfoCache objects in the scene to
1, none flagged `retrieved`, and `UnitManager` carries no `guid`, `uid` or
`unitId` to fall back on. For caches the cell therefore is the detector, by
elimination.

Cache cells are consequently SHIPPED rather than observed - `MapCells.cs`,
generated by `tools/gen-mapcells.py` from a 20-mission dump, 17 missions and 20
cells, cross-checked per row against the apworld's `INSTANCE_COUNTS` by
`audit.py`. Positions are map data, so knowing them beats waiting to see them:
an observed-set memory can never help a save whose caches were taken before the
mod was installed. That memory survives as a fallback, with counting under it.

All 20 missions dumped: 203 structures, zero unreadable cells, and no two
structures of one kind sharing a cell. All three kinds read the same
`UnitManager.cellX`/`cellY` - all 63 totems resolved through the unit on the same
object, so they share one coordinate space with the enemies, which is what makes
"near the enemies that need to be nullified" a measurable claim rather than a
figure of speech.

## 1. Farsite

- **Instances (2026-09-08).** Caches at cells (66,51) and (150,63) -> instances
  1 and 2. Instance 1 is the free one: "The one nearest to the rift lab is the
  only free one. since you can't move the rift lab you need a weapon to get to
  the 2nd item (right side)", and (150,63) is the right-hand one. So
  `WAIVES_INSTANCE` keeps index 1, now for a derived reason.
...
- **Instances (2026-09-08).** Totems at (54,48), (201,141), (18,185) ->
  instances 1, 2, 3. The one needing Porter-or-Platform is **instance 3**: "it's
  the one in the top left, near the enemies that need to be nullified". (18,185)
  is 21 cells from the nearest nullify target where the other two are 96 and 86
  away, and it is the low-X high-Y corner - both criteria pick the same totem.
...
- **Instances (2026-09-08).** This SUPERSEDES the earlier three-tier reading
  ("2 of the enemies ... 7 more ... the final 5 in darkness"), which was correct
  about the darkness count but too coarse about the rest. Nullify cells sort to:

  | instances | group | cells | requirement |
  |---|---|---|---|
  | 1-4 | bottom-right | (198,25)E (199,40)Skimmer (233,45)E (244,55)Blob | Sniper; no mover - "you can just get to these with a tower" |
  | 5,7-10 | under the dark tower, centre-right | (209,73)Spore (194,88)Denier (204,88)E (180,102)Skimmer (209,103)Spore | + Chronat |
  | 6,11 | top-left creep emitters | (17,76)E (35,124)E | nothing beyond a weapon and the nullifier |
  | 12-14 | top-right | (165,132)E (212,135)E (146,144)Blob | + Chronat OR Pylon/Porter/Platform |

  The described types match the dumped types exactly in all four groups, which
  is what makes this a derivation. The dark group is five with the Denier
  included, which is how it reconciles with "the final 5 enemies are in
  darkness".

  Caches at (239,89) and (89,125) -> instances 1 and 2. Instance 1 is the buried
  right-side one (TERP); instance 2 is "the item on the top left middle", which
  needs only a weapon - so the Terp is keyed to instance 1 and is no longer
  claimed for both.

  **Miner is deliberately SOFT here.** The older note wanted "miner access to
  get to the RESO"; the fuller reading above does not mention a Miner at all.
  Dropping it would be a loosening, and a wrong loosening shows a check green
  that cannot be reached; keeping it hard would paint reachable checks red. Soft
  means logic still assumes it and the map does not call it unreachable. Open
  for the designer to strike.
- Helped but not needed:
- Objective notes:
- Blockers:
...
- **Requirements restated (2026-09-08), and the tier WITHDRAWN.** "You Need
  Snipers to do anything in this level. I was able to beat it with Miners,
  Snipers, and Cannons only... You could put just Snipers and Cannons as out of
  logic (yellow) as it might be possible. I think Mortars and Snipers only is
  not possible (red)... Ignore my 'first 2/4 enemies' line."

  So: green needs Cannon + Sniper + Miner, yellow is Cannon + Sniper, and red is
  anything without a Cannon or without a Sniper. **Cannon specifically** - the
  mortar-only route is gone, which tightens what the generator may assume for
  the whole mission, not just its nullifies. Miner sits in `MISSION_SOFT`.

  The old escalating tier ("the first 2, maybe even 4, with less") was an ORDER
  claim and cannot survive per-structure identity; it is withdrawn rather than
  re-derived, so all nine targets carry one rule. Nullify cells for the record:
  (77,61) (113,61) (39,81) (150,81) (78,104) (112,104) (16,115) (170,115)
  (95,131); totems (14,74) and (178,74); cache (95,120).
- Helped but not needed:
- Objective notes:
- Blockers:
