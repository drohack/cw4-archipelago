...
This file is the methodology and the results log, and is the only copy. A
presentable HTML version was versioned here for a while so it could be
republished; it was deleted on 2026-09-16, frozen four figures out of date,
because a report that has to be regenerated by hand after every change is a
report that stops being true and keeps looking authoritative.
...
effort and hides errors. `create_all_items` is straight-line arithmetic:
    ...
    real      = (20 - starters) + (units - retired) + bonus + progressive_erns
    ernUpg    = 12 names * min(ern_upgrade_copies, 4)
    energy    = energy_storage_copies + base_generation_copies
    remaining = 236 - real - ernUpg - energy
    traps     = remaining * trap_percentage // 100   (0 if every weight is 0)
    filler    = remaining - traps

`retired` is the subtraction that is easy to miss: a retired name keeps its id
forever - ids are positional - and is never generated, so it is in `UNIT_ITEMS`
and not in the pool. Without it every configuration derives one real item too
many and one filler too few.

The ERN upgrade and energy blocks come off the top like the real items, not out
of the filler remainder, because both are generated as FIXED counts before the
trap split.
...
`derive.py` runs that against all 16 single-player configurations in its `CASES`
list and compares it with real generated seeds. Currently 16 of 16 match exactly. Generation is the
