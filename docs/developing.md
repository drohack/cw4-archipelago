...
2. **apworld tests** (in the Archipelago clone): `tools/ap-sync.ps1` then
   ...
   **Keep the clone at `minimum_ap_version`, not on main.** It tracked main for
   ...
   Every Archipelago tree on this machine should report that version -
...
- `Creeper World 4.yaml` - a ready-to-use options file, generated from the
  ...
  **Generated by the Archipelago in `minimum_ap_version`, not by the dev
  clone.** The clone tracks main, so the template was being produced by a
  version nobody can install. That was found by a player comparing our shipped
  yaml against one their own Archipelago generated: a core option's example text
  differed. Cosmetic in that instance - the only diffs were one comment and a
  trailing newline - but the same mechanism would ship an option that exists
  only upstream, which a supported Archipelago would not understand. This used
  to be handled by rewriting the `version:` line afterwards, which corrected
  what the file CLAIMED and not what it contained. `package-release.ps1` now
  builds it in a git worktree of the clone checked out at that tag, asserts the
  worktree really reports that version, and asserts the generated file declares
  it.
