...
- **The game starts but there is no Archipelago panel, and the BepInEx console
  ...
  throws away. Add `steam_appid.txt` containing `848480` next to `CW4.exe` and
  launch the exe directly rather than from Steam; see Troubleshooting for why.
  Nothing is corrupt, and reinstalling BepInEx or the game does not help: a
  clean BepInEx reinstall reproduced it exactly.
...
### BepInEx runs, then the console closes before the game window opens

This is the one that looks like BepInEx is broken when it is not: the mod loads
correctly and the process it loaded into is then thrown away.
...
Creeper World 4 asks Steamworks whether it needs restarting, and
...
- **Game starts but no panel / nothing changed**: check
  ...
  - File missing entirely: BepInEx is not installed (step 1), or the game was
    launched from Steam rather than by running the exe (see the console entry
    above).
  - File present and the line IS there, but no panel in game: the mod loaded
    into a process the game then replaced - add `steam_appid.txt`, as above.
