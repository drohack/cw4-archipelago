...
# The subject: a factory holds the mined stock a cache adds to.
require_spawn factory 2
...
# WHICH WRITE DOES THE SIM HONOUR? That is the whole question.
#
# A cache only needs to ADD stock, so this needs no mining economy at all - the
# earlier Mine Production work needed one because it measured a RATE.
#
# Both write methods are tried because guessing which field the sim reads has
# been wrong repeatedly here: gs.energyStore is a recomputed summary, and
# GetWareHeld reads the factory's INPUT slots rather than its output stock.
fire() {
  local args="$1"
  mark
  printf '%s\n' "boon:cache $args" > "$CMD"
  if ! wait_since "BOON resource cache:" 25; then
    echo "  NO LOG for 'boon:cache $args'"
    return
  fi
  since | grep -a "BOON resource cache:" | sed 's/^.*BOON/  BOON/'
}

say "0. CHEAT STATE"
dev "set:infiniteresources=off"
sleep 2
echo "  infiniteresources must be OFF - it tops up held wares"

say "1. WARE IDS THIS MISSION DEFINES"
# -1 means the mission does not define that ware, which is a legitimate whiff
# rather than a bug.
for W in bluite redon liftic greenar; do
  fire "$W 0"
done

say "2. STACKING - on LIFTIC, the ware this mission actually has a channel for"
# The first version of this section used bluite, which has NO channel on story2
# - so it was testing a whiff and would have reported "stacking broken". Test
# the write on a ware the mission supports.
fire "liftic 10"
fire "liftic 10"
echo "  (expect 10 then 20: the second must ADD, not replace)"

say "3. CreateProducedWares - the other write, same ware"
fire "liftic 5 create"
echo "  (expect 25 if Create also lands)"

say "4. THE CAP - Factory.MAX_WARES is 360"
fire "liftic 99999"
echo "  (expect exactly 360, never above)"
fire "liftic 10"
echo "  (expect 'already at 360 of cap 360, nothing to add')"

say "4b. ALL CHANNELLED WARES AT ONCE - the single-item design"
fire "all 5"
echo "  (expect a grant only where a channel exists, whiffs named for the rest)"

say "5. THE WHIFF - no factory"
echo "  destroying the factories, then firing again"
send "ern:dumpall factory" "DUMPALL "
since | grep -ac "DUMPALL   " | sed 's/^/    factory fields dumped: /'
echo "  (a whiff must read as allowed, not as a bug)"
