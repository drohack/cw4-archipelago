#!/bin/bash
# Move Speed: relocate a cannon and time the trip.
#
...
# MOVE SPEED (upgrade 3)
#
# The designer's correction, and it was right: a cannon CAN be moved, so this
# upgrade is testable after all. An earlier run wrote it off after watching a
# STATIONARY cannon's fields and seeing nothing move - which is true and
# useless, because MOVE_SPEED stays at 0.09 whatever the upgrade does. The
# multiplier is applied while stepping, so the trip has to be timed.
#
# Reported as cells/tick so the two levels are directly comparable even if the
# legal target lands at a different distance.
one_move() {
  local label="$1" i out=""
  mark
  printf '%s
' "measure:move 12" > "$CMD"
  for i in $(seq 1 45); do
    sleep 2
    out=$(since | grep -a "MEASURE move: result=" | tail -1)
    [ -n "$out" ] && break
  done
  if [ -n "$out" ]; then
    echo "  [$label] ${out#*MEASURE move: }"
  else
    echo "  [$label] NO RESULT - the mod never reported"
  fi
}
...
dev "set:instantbuild=on"; dev "set:infiniteresources=on"
sleep 3
one_move "0 percent a"; one_move "0 percent b"
to_full 3
one_move "100 percent a"; one_move "100 percent b"
boost_to_max "Move Speed" 3
one_move "200 percent a"; one_move "200 percent b"
