...
# THE ENERGY DRAIN TRAP was writing gs.energyStore, which EnergyGranter's own
# header calls a summary the sim recomputes every tick - so the trap moved the
# HUD and took nothing. Its mirror, Energy Cache, wrote commandBase.ammo all
# along, which is what exposed the mismatch.
#
# The test therefore reads BOTH fields every time. Reading only the one the
# effect writes is what let this survive: a drained summary looks exactly like
# a working trap.
store() {
  local d
  d=$(dumpone commandbase)
  printf 'cb.ammo=%s cb.MAX_AMMO=%s' "$(field ammo "$d")" "$(field MAX_AMMO "$d")"
}
summary() {
  local d
  d=$(dumpone gs)
  printf 'gs.energyStore=%s' "$(field energyStore "$d")"
}

say "0. CHEAT STATE - infiniteresources MUST be off"
dev "set:infiniteresources=off"
sleep 3
echo "  $(store)"
echo "  a MAX_AMMO of 100000 means the cheat is still on - stop here"

say "1. LET THE STORE BUILD UP"
for i in 1 2 3; do
  sleep 6
  echo "  $(store)   $(summary)"
done

say "2. DRAIN IT - 100 percent"
send "trap:energy 1.0" "TRAP energy:"
since | grep -a "TRAP energy:" | tail -1 | sed 's/^.*TRAP/  TRAP/'
echo "  immediately after: $(store)"
echo "                     $(summary)"
echo "  VERDICT: cb.ammo must have DROPPED to ~0. If only gs.energyStore moved,"
echo "  the trap is still writing the summary and taking nothing."

say "3. IS IT RECOVERABLE? (a trap may sting, never end the mission)"
for i in 1 2 3 4; do
  sleep 8
  echo "  t+$((i*8))s  $(store)"
done
echo "  VERDICT: energy must be climbing back on its own"

say "4. HALF DRAIN - the proportional case"
sleep 10
B=$(store); echo "  before: $B"
send "trap:energy 0.5" "TRAP energy:"
since | grep -a "TRAP energy:" | tail -1 | sed 's/^.*TRAP/  TRAP/'
echo "  after:  $(store)"
echo "  VERDICT: roughly half of what was there, not all of it"

say "5. NO RIFT LAB - must not throw"
echo "  (the lab is placed here, so this path is covered by the code guard;"
echo "   a campaign mission starts with the lab in hand and cb is null then)"
